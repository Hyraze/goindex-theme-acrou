...
var authConfig = {
   ...
   * [protect_file_link]: Whether Basic Auth is used to protect file links, the default value (when not set) is false, that is, file links are not protected (for direct link download/external playback, etc.)
   ...
   * If you want to protect the file link, you need to set protect_file_link to true. If you want to perform external playback, you need to replace host with user:pass@host
   ...
   * If the value set is too small, it will cause the incremental loading (paged loading) of the scroll bar of the file list page to fail;
   * Another effect of this value is that if the number of files in the directory is greater than this setting value (that is, multiple pages are required), the results of the first listing of the directory will be cached.
   */
   ...
   * [Note] If the password verification for the .password file is enabled, every time the directory is listed, the overhead of querying whether the .password file exists in the directory will be added.
   */
